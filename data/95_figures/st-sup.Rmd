...
Figure S15. Identification of cis-regulatory variants associated with variable cold responsive pattern in a panel of 25 maize genotypes.  (A) Consistency of log2 fold changes for each gene called between two experiments: the replicated HY experiment with 3 maize inbreds (x-axis) and the NM experiment with 25 maize genotypes (y-axis).  Point density is shown to avoid over plotting.  Linear regression is performed and the adjusted R2 values are marked.  (B) Cis-regulated variable response genes are enriched in having significantly associated local variants.  Shown are proportions of genes with detected local associations for variable response genes under different regulatory patterns.
